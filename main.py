...
def strip_channel_id(channel_id: int) -> int:
...
async def get_channel_peer(channel_id: int):
    stripped = strip_channel_id(channel_id)
    try:
        result = await app.invoke(raw.functions.messages.GetDialogs(
            offset_date=0,
            offset_id=0,
        ...
        for chat in result.chats:
            if hasattr(chat, "id") and chat.id == stripped:
                print(f"[{ts()}] Канал найден, access_hash получен")
                return raw.types.InputChannel(channel_id=chat.id, access_hash=chat.access_hash)
    except Exception as e:
        print(f"[{ts()}] Ошибка получения диалогов: {e}")
    ...
    try:
        ...
        for chat in result.chats:
            if hasattr(chat, "id") and chat.id == stripped:
                print(f"[{ts()}] Канал найден в расширенном поиске")
                return raw.types.InputChannel(channel_id=chat.id, access_hash=chat.access_hash)
    except Exception as e:
        print(f"[{ts()}] Ошибка расширенного поиска: {e}")

    print(f"[{ts()}] Канал не найден в диалогах!")
    return None
...
async def ensure_channel() -> int:
    ...
    if channel_id:
        print(f"[{ts()}] Канал: {channel_id}, получаю peer...")
        channel_peer = await get_channel_peer(channel_id)
        if channel_peer is None:
            print(f"[{ts()}] ОШИБКА: убедись что аккаунт является админом канала {channel_id}")
        return channel_id
    ...
    channel_peer = await get_channel_peer(chat.id)
...
async def snipe_loop(username: str, channel_id: int):
    global is_running, target_username, snipe_task, channel_peer
    ...
    if channel_peer is None:
        print(f"[{ts()}] Не удалось получить peer канала, останавливаю снайпинг")
        await app.send_message(
            ADMIN_CHAT_ID,
            f'<tg-emoji emoji-id="5870657884844462243">❌</tg-emoji> Не удалось получить доступ к каналу. Проверь CHANNEL_ID и права администратора.',
        )
        is_running = False
